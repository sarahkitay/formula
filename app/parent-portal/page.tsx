...
import type { ParentPlayerLinkRow, PlayerRow } from '@/types/players'
...
export default function ParentPortalHubPage() {
  ...
  const [links, setLinks] = useState<ParentPlayerLinkRow[]>([])
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      if (ppErr) {
        ...
        setLinks((ppData as ParentPlayerLinkRow[]) ?? [])
        setLinksError(null)
      }
  ...
  if (phase === 'loading' || !profile) {
    return (
      <div className="portal-brand-surface admin-os flex min-h-dvh flex-col items-center justify-center gap-3 text-formula-mist">
        <p className="font-mono text-[10px] uppercase tracking-[0.24em]">Loading parent portal…</p>
      </div>
    )
  }
  ...
  return (
    <div className="portal-brand-surface admin-os min-h-dvh text-formula-paper">
      <header className="flex flex-wrap items-center justify-between gap-4 border-b border-formula-frost/10 px-6 py-5">
        <div>
          <p className="font-mono text-[10px] uppercase tracking-[0.22em] text-formula-mist">Parent portal</p>
          <h1 className="mt-1 text-xl font-semibold tracking-tight text-formula-paper">Welcome</h1>
        </div>
      ...
      <main className="mx-auto max-w-2xl space-y-8 px-6 py-10">
        <section className="border border-formula-frost/15 bg-formula-paper/[0.03] p-6">
          <h2 className="font-mono text-[10px] font-semibold uppercase tracking-[0.2em] text-formula-mist">Account</h2>
          <p className="mt-3 text-lg text-formula-paper">{displayName}</p>
        </section>

        <section className="border border-formula-frost/15 bg-formula-paper/[0.03] p-6">
          <h2 className="font-mono text-[10px] font-semibold uppercase tracking-[0.2em] text-formula-mist">Linked athletes</h2>
          {linksError ? (
            <p className="mt-4 text-sm text-amber-200/90">
              Could not load links ({linksError}). If tables are missing, run <code className="text-formula-volt">supabase/parent_players_players.sql</code> in the
              Supabase SQL editor.
            </p>
          ) : links.length === 0 ? (
            <p className="mt-4 text-sm text-formula-frost/80">
              No athletes linked yet. Add a row in <code className="text-formula-volt">parent_players</code> for this parent in Supabase.
            </p>
          ) : (
            <ul className="mt-4 space-y-6">
              {links.map((row) => {
                const player = embedOne<PlayerRow>(row.players)
                const name = player
                  ? [player.first_name, player.last_name].filter(Boolean).join(' ') || 'Athlete'
                  : 'Unknown athlete'
                return (
                  <li key={row.id} className="border-t border-formula-frost/10 pt-4 first:border-t-0 first:pt-0">
                    <p className="font-medium text-formula-paper">{name}</p>
                    <dl className="mt-2 grid gap-1 text-sm text-formula-frost/85">
                      <div className="flex gap-2">
                        <dt className="font-mono text-[10px] uppercase tracking-[0.14em] text-formula-mist">Age group</dt>
                        <dd>{player?.age_group ?? '—'}</dd>
                      </div>
                      <div className="flex gap-2">
                        <dt className="font-mono text-[10px] uppercase tracking-[0.14em] text-formula-mist">Active program</dt>
                        <dd>—</dd>
                      </div>
                      <div className="flex gap-2">
                        <dt className="font-mono text-[10px] uppercase tracking-[0.14em] text-formula-mist">Latest assessment</dt>
                        <dd>—</dd>
                      </div>
                    </dl>
                  </li>
